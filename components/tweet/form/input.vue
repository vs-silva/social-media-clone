  ...
  <div>
    <div class="flex items-center flex-shrink-0 p-4 pb-0">
      ...
      <div class="w-full p-2">
        <textarea class="w-full h-10 text-lg text-gray-900 placeholder:text-gray-400 bg-transparent border-0 dark:tex.white focus:ring-0"
                  v-model="tweetText" placeholder="What's happening ?"></textarea>
      </div>
    ...
    <div class="p-4 pl-16">
      ...
      <img :src="mediaFileURL" v-if="mediaFileURL" alt="" class="rounded-2xl" :class="`${twitterBorderColor}`"/>

      <input type="file"
             ref="mediaInput"
             hidden
             accept="image/png, image/gif, image/jpeg"
             @change.prevent="(event) => handleMediaChange(event.target as HTMLInputElement)"
      />
    ...
    <div class="flex p-2 pl-14">
      ...
      <div class="p-2 text-blue-400 rounded-full hover:bg-blue-50 dark:hover:bg-dim-800 cursor-pointer" @click.prevent="handleMediaInputClick">
    ...
    <div>
      <button @click.prevent="() => emits(TweetFormEventTypeConstants.SUBMIT_TWEET, { tweetText, mediaFile })">Tweet</button>
    </div>
...
import {TweetFormEventTypeConstants} from "../constants/tweet-form-event-type.constants";
import useTailwindConfig from "../../../composables/use-tailwind-config";
...
const emits = defineEmits([
  TweetFormEventTypeConstants.SUBMIT_TWEET
]);

function handleMediaInputClick(): void {
  mediaInput.value.click();
}

function handleMediaChange(event: HTMLInputElement):void {
  ...
  reader.onload = (event) => {
    mediaFileURL.value = event?.target?.result as string;
  };
